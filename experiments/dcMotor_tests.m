...
X = zeros(N-output_max_delay,output_max_delay+input_max_delay1+input_max_delay2);
e = zeros(N-output_max_delay,1);

for i = 1:(N-output_max_delay)
    e(i) = y(i+output_max_delay);
    X(i,:) = [ flip( y( i : (i+output_max_delay-1)) ) , ...
               flip( u1( i : (i+input_max_delay1-1) ) ), ...
               flip( u2( i : (i+input_max_delay2-1) ) )];
end
...
Ud1 = zeros(input_max_delay1,1);
Ud2 = zeros(input_max_delay2,1);
Yd = zeros(output_max_delay,1);

% Vectors with coefficients

A = v(1:output_max_delay)';
B1 = v(output_max_delay+1:output_max_delay+input_max_delay1)';
B2 = v(output_max_delay+input_max_delay1+1:end)';
...
for i = 1:N
    ...
    % Predicted output (model)
    yh(i) = A*Yd + B1*Ud1 + B2*Ud2;
    
    % Uptade memory
    Ud1 = [u1(i);Ud1(1:end-1)];
    Ud2 = [u2(i);Ud2(1:end-1)];
    Yd = [y(i);Yd(1:end-1)];
