...
OPT.file = 'wsom2018_f6.mat'; % file where all the variables will be saved  
...
PAR_ksom_gd2 = ksom_gd_ald_train(DATAtr,PAR_ksom_gd2);

PAR_ksom_ef1 = ksom_ef_train(DATAtr,PAR_ksom_ef1);

PAR_ksom_ef2 = ksom_ef_ald_train(DATAtr,PAR_ksom_ef2);
...
% KSOM-GD-ALD (Gaussian)

[OUTtr] = ksom_gd_ald_classify(DATAtr,PAR_ksom_gd2);
OUTtr.nf = normal_or_fail(OUTtr.Mconf);
ksomgd2_out_tr{r,1} = OUTtr;                            % training set results

[OUTts] = ksom_gd_ald_classify(DATAts,PAR_ksom_gd2);
OUTts.nf = normal_or_fail(OUTts.Mconf);
...
KSOM3p_acc{r} = PAR_ksom_ef1;                           % hold parameters
ksomef1_Mconf_sum = ksomef1_Mconf_sum + OUTts.Mconf;    % hold confusion matrix

% KSOM-EF-ALD (Gaussian)

[OUTtr] = ksom_ef_ald_classify(DATAtr,PAR_ksom_ef2);
OUTtr.nf = normal_or_fail(OUTtr.Mconf);
ksomef2_out_tr{r,1} = OUTtr;                            % training set results

[OUTts] = ksom_ef_ald_classify(DATAts,PAR_ksom_ef2);
OUTts.nf = normal_or_fail(OUTts.Mconf);
