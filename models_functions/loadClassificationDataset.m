function [dataset] = loadClassificationDataset(dataset_name)

% --- Selects a Data Set for Classification ---
%
%   dataset = loadClassificationDataset(dataset_name)
%
%   Input:
%       dataset_name = which data base will be used
%           'iris'
%           'dermatology'
...
if(strcmp(dataset_name,'iris'))
    ...
    dataset.input = loaded_data(:,1:end-1)';
    dataset.output = loaded_data(:,end)';
    dataset.lbl = dataset.output;
    dataset.name = 'iris';
elseif(strcmp(dataset_name,'dermatology'))
