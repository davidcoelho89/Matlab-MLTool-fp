...
OPT.prob = 'LPC';	% Choose Dataset ('LPC' or 'PSD')
...
HPpca.tol = 1;      % Tolerance [0 - 1] (not used)
...
if(strcmp(OPT.prob,'LPC'))
    ...
    DATAtr = loadedDATA.DATAlpc;
    DATAtr.lbl = DATAtr.output;
    loadedDATA = load("DATAlpc_david.mat");
    DATAts = loadedDATA.DATAlpc_david;
    DATAts.lbl = DATAts.output;

elseif(strcmp(OPT.prob,'PSD'))
    loadedDATA = load("DATApsd.mat");
    DATAtr = loadedDATA.DATApsd;
    DATAtr.lbl = DATAtr.output;
    loadedDATA = load("DATApsd_david.mat");
    DATAts = loadedDATA.DATApsd_david;
    DATAts.lbl = DATAts.output;
...
if(OPT.bc == 1)
    DATAtr.input = ((DATAtr.input).^(OPT.gamma))./(OPT.gamma);
    DATAts.input = ((DATAts.input).^(OPT.gamma))./(OPT.gamma);
end
