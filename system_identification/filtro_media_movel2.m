...
% Media centralizada na amostra atual
...
for n=offset+1:length(xn)-offset
   xf1(n) = mean(xn(n-offset:n+offset));
end
...
for n=offset+1:length(xf1)-offset
   xf2(n) = mean(xf1(n-offset:n+offset));
end
...
figure; 
subplot(2,2,1); plot(t(1:200), x(1:200),'linewidth',3);  % mostra primeiros ciclos do sinal
title('SINAL SEM RUIDO'); grid

subplot(2,2,2); plot(t(1:200), xn(1:200),'linewidth',3);  % mostra primeiros ciclos do sinal ruidoso
title('SINAL COM RUIDO GAUSSIANO'); grid

subplot(2,2,3); plot(t(1:200), xf1(1:200),'linewidth',3);  % mostra primeiros ciclos do sinal filtrado 1
title('SINAL FILTRADO (PASSADA 1)'); grid

subplot(2,2,4); plot(t(1:200), xf2(1:200),'linewidth',3);  % mostra primeiros ciclos do sinal filtrado 2
